...
- `images/logo-watermark.png` — a very low-opacity (~5%) version of the logo,
  set as a fixed, centered background on `<body>` in `style.css` so it shows
  faintly behind content on every page without competing with photos or cards.
